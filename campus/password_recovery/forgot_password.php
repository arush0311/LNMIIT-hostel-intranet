...
<?php
  if(isset($_POST['submit']) and @$_POST['username'])
  {
      ...
      $query="SELECT `Emailid` FROM `campus_student_info` WHERE `Rollno`='$roll_no'";
      ...
      if(mysql_num_rows($query_run)==1)
      {
        ...
        $mail->Subject = 'Forgot password of '.$roll_no;
        $mail->Body    = '<b>Hi '.$roll_no.',</b><div><font color="black"><br>Click <a href="http://intranet.lnmiit.ac.in/portal/password_recovery/recover.php?token='.$token.'&username='.$roll_no.'">here</a> reset your password<br> <br/>
          </font>
          </div>';
      
        if(!$mail->send()) {
          echo '<b><center>Your password reset link Couldnot be  send to your e-mail address.ERROR..</center></b>';
          exit;
        }
        else{
          echo '<b><center>Your password reset link send to your e-mail address</center></b>';
        }
